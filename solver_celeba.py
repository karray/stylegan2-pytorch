...
from model import Generator1, DiscriminatorMini, MappingNetwork, Encoder
...
def train(args, loader, generator, discriminator,mapping, encoder, g_optim, d_optim, f_optim, e_optim, g_ema, debug_images, debug_z, debug_noise, device,
    ...
    for idx in pbar:
        ...
        # fake_img, _ = generator([encoder(real_img.detach())])

        # # if args.augment:
        # #     fake_img, _ = augment(fake_img, ada_aug_p)

        # rec_loss = torch.abs(real_img - fake_img).mean()
        # loss_dict["reconstruction_loss"] = rec_loss
        ...
        # reconstruction_loss = loss_reduced["reconstruction_loss"].mean().item()
        ...
        if get_rank() == 0:
                ...
                wandb.log(
                    {
                        ...
                        'w_consitency_loss': w_consitency_loss,
            ...
            if (i+1) % 100 == 0:
                torch.save(
                    ...
                    ospj(working_dir, f"checkpoint_{str(i).zfill(6)}.pt.tar"),
                )
                torch.save(
                    {
                        "g": g_module.state_dict(),
                    ...
                    ospj(working_dir, 'checkpoint_last.pt.tar'),
                )
                ...
                with torch.no_grad():
                    ...
                    if wandb and args.wandb:
                        wandb.save(ospj(working_dir, 'checkpoint_last.pt.tar'))
                        wandb.log({'debug_img': wandb.Image(
                            utils.make_grid(debug_img, nrow=8, padding=0, normalize=True, range=(-1, 1)).permute(1, 2, 0).cpu().numpy())},
                                    step=i)
...
if __name__ == "__main__":
    ...
    train(args, loader, generator, discriminator, mapping, encoder, g_optim, d_optim, f_optim, e_optim, g_ema, debug_images, debug_z, debug_noise, device,
            sample_dir, working_dir)
